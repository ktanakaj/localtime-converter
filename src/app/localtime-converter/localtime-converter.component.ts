 ...
 */
@Component({
  selector: 'app-localtime-converter',
  templateUrl: './localtime-converter.component.html',
...
export class LocaltimeConverterComponent implements OnInit {
  ...
  reset(): void {
    ...
    this.offsetMin = moment.tz.zone(timezone)?.offset(Date.now()) || 0;
  ...
  convert(): void {
    ...
    const offsetMin = moment.tz.zone(this.form.timezone || '')?.offset(date.getTime()) || 0;
    this.result = {
